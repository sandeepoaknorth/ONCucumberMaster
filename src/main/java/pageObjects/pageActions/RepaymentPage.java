...
public class RepaymentPage extends pageObjects.pageObject.RepaymentPage {
	...
	public void checkCommonFields_group1() {
		...
		softAssert.assertEquals(getNormalRepaymentModalButton().isDisplayed(), true,
	            "Normal Repayment Button is not visible in Advance Repayment Modal for Repayment Method: " + getRepaymentMethod().getText());
		
		softAssert.assertEquals(getCancelModalButton().isDisplayed(), true,
                "Cancel Repayment Button is not visible in Advance Repayment Modal for Repayment Method: " + getRepaymentMethod().getText());
		
		softAssert.assertEquals(getRemarks().isDisplayed(), true,
                "Remarks text field is not visible in Advance Repayment Modal for Repayment Method: " + getRepaymentMethod().getText());
	...
	public void checkCommonFields_group2() throws InterruptedException {
		...
		softAssert.assertEquals(getFinalRepaymentAmountLabel().getText(), prop.getProperty("final_repayment_amount_label"), 
				"Final Repayment Amount label verbiage is not correct.");
		
		softAssert.assertEquals(getTotalApportionedAmountLabel().getText(), prop.getProperty("total_apportioned_amount_label"), 
		...
		softAssert.assertEquals(getRepaymentDate().isDisplayed(), true,
	            "Repayment date field is not visible in Advance Repayment Modal for Repayment Method: " + getRepaymentMethod().getText());
		
		softAssert.assertEquals(getRepaymentAmount().isDisplayed(), true,
                "Repayment amount field is not visible in Advance Repayment Modal for Repayment Method: " + getRepaymentMethod().getText());
		
		softAssert.assertEquals(getShowDuesButton().isEnabled(), false,
                "Show Dues button is enabled in Advance Repayment Modal for Repayment Method before selecting date: " + getRepaymentMethod().getText());
		...
		for(int i=0; i<getApportioningMethodDropDownValues().size(); i++) {
			softAssert.assertEquals(getApportioningMethodDropDownValues().get(i).getText(), prop.getProperty("apportioning_method"+(i+1)), "Apportioning method "+prop.getProperty("apportioning_method"+(i+1))+" verbiage is not correct in Advance Repayment Section.");
			softAssert.assertAll();
		}
		softAssert.assertAll();
	}
