...
public class Customermainpage extends BasePage {
	...
	@FindBy(xpath = "//fims-data-table//table//tbody")
	private WebElement webtablebody;

	@FindBy(xpath = "//fims-data-table//table//thead")
	private WebElement webtablehead;

	@FindBy(xpath = "//tbody/tr/td[5]")
	private List<WebElement> accountStatus;

	@FindBy(xpath = "//div[contains(@id,'mat-tab-label-0-')]")
	private WebElement loanSection;
	...
	public WebElement getAccountView(String accountStatus) throws InterruptedException {
		...
		for (int i = 0; i <= accountStatusSize; i++) {
			...
			if (getAccountStatus().get(i).getText().equalsIgnoreCase(accountStatus)) {
				getAccountActionView = getDriver()
						.findElement(By.xpath("(//tbody/tr/td[5])[" + (i + 1) + "]/following-sibling::td[2]"));
				break;
			}
		}
